...
export class Pagamento1728395035017 implements MigrationInterface {
  name = 'Pagamento1728395035017';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "pagamentos" ("id" SERIAL NOT NULL, "valor" integer NOT NULL, "descrição" character varying NOT NULL, CONSTRAINT "PK_0127f8bc8386b0e522c7cc5a9fc" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `ALTER TABLE "pedidos" ADD "payment_id" integer NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TYPE "public"."pedidos_status_enum" RENAME TO "pedidos_status_enum_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."pedidos_status_enum" AS ENUM('pendente', 'recebido', 'em preparação', 'aproveed', 'finalizado', 'rejected')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."pedidos_status_enum_old" AS ENUM('pendente', 'recebido', 'em preparação', 'pronto', 'finalizado', 'cancelado')`,
    );
    await queryRunner.query(
      `ALTER TABLE "pedidos" ALTER COLUMN "status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(`ALTER TABLE "pedidos" DROP COLUMN "payment_id"`);
